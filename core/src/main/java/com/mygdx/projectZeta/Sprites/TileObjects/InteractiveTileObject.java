...
public abstract class InteractiveTileObject {
    ...
    public InteractiveTileObject(PlayScreen screen, MapObject object){
        ...
        bdef.position.set((bounds.getX() + bounds.getWidth() /2) / projectZeta.PPM, (bounds.getY() + bounds.getHeight() /2)/ projectZeta.PPM);

        body = world.createBody(bdef);
        objectStateTimer = 0;

        shape.setAsBox(bounds.getWidth() /2 / projectZeta.PPM, bounds.getHeight() / 2 / projectZeta.PPM);
        fdef.shape = shape;
    ...
    public TiledMapTileLayer.Cell getCell(){
        ...
        return layer.getCell((int) (body.getPosition().x * projectZeta.PPM / 16),
                (int) (body.getPosition().y * projectZeta.PPM / 16));
    }
